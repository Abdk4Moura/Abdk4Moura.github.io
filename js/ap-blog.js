...
(function () {
  ...
  fetch("posts/posts.json")
    ...
    .then((files) => Promise.all(files.map((f) =>
      fetch("posts/" + f).then((r) => r.text()).then((t) => Object.assign(meta(t), { file: f })).catch(() => null)
    )))
    .then((list) => {
      list = list.filter(Boolean).sort((a, b) => new Date(b.date) - new Date(a.date));
      if (!list.length) { box.innerHTML = '<div class="bl-empty">No posts yet.</div>'; return; }
      box.innerHTML = "";
      list.forEach((p, i) => {
    ...
    .catch(() => { box.innerHTML = '<div class="bl-empty">Could not load posts.</div>'; });
})();
